...
public class SmsCodeLoginImpl implements SmsService {
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                if (StringUtils.isNotBlank(result)) {
                    ...
                    if ("0".equals(jsonObject.getString("code"))) {
                        stringRedisTemplate.boundValueOps(RedisKey.KEY_SMS_CODE_LOGIN + ":" + phone).set(random, 1, TimeUnit.MINUTES);
                        return true;
                    }
                }
            }
    ...
    public boolean checkSmsCode(String phone, String code) {
        if (stringRedisTemplate.hasKey(RedisKey.KEY_SMS_CODE_LOGIN + ":" + phone)) {
            String querySmsCode = stringRedisTemplate.boundValueOps(RedisKey.KEY_SMS_CODE_LOGIN + ":" + phone).get();
